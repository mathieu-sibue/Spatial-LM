...
class LayoutLMv3TextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        embeddings_spatial = self.LayerNorm(spatial_position_embeddings)
        embeddings_spatial = self.dropout(embeddings_spatial)

        return embeddings, embeddings_spatial
...
class LayoutLMv3SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if spatial_QKd is not None:
            attention_scores += spatial_QKd
...
class LayoutLMv3Attention(nn.Module):
    def __init__(self, config):
        ...
        self.self_spatial = LayoutLMv3SelfAttention(config)
        self.output = LayoutLMv3SelfOutput(config)
        if config.spatial_attention_update:
            self.output_spatial = LayoutLMv3SelfOutput(config)
    ...
    def forward(
        ...
        # add spatial self-attention
        spatial_self_output = self.self_spatial(
            hidden_states_spatial,
            attention_mask,
            head_mask,
            output_attentions = True,   # fix to return QKd
            rel_pos= None,  # fix to cancel bias
            rel_2d_pos=None,    # fix to cancel bias
        )
        spatial_QKd = spatial_self_output[2]    # the third one
        ...
        self_outputs = self.self(
            ...
            spatial_QKd = spatial_QKd
        )
        attention_output = self.output(self_outputs[0], hidden_states)

        if self.config.spatial_attention_update:
            spatial_output = self.output_spatial(spatial_self_output[0],hidden_states_spatial)
            outputs = (attention_output,spatial_output)
        else:
...
class LayoutLMv3Head(nn.Module):
...
class DesentForBinaryQA(LayoutLMv3PreTrainedModel):
    ...
    def forward(
        ...
        # sequence_output = outputs[1]    # get the pooled results
        sequence_output = outputs[0][:, 0, :]   # 0 is the first hidden param, (B, S=0, D)
        logits = self.qa_outputs(sequence_output)
